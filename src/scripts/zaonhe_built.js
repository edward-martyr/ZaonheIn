import { 音韻地位 as class音韻地位, query字頭 }  from "qieyun";

const schemas = {};

function perform(input, 音韻地位, 字頭, 選項) {
  音韻地位 = 音韻地位 instanceof class音韻地位 ? 音韻地位 : null;
 ...
 * @return {{音韻地位: class音韻地位, 解釋: string, 擬音: string | string[]}[]} 音韻地位、解釋、音韻地位對應的擬音或擬音陣列
 */
export function from字頭(schema, 字頭, 選項) {
  return query字頭(字頭).map(result => ({
    ...result,
    擬音: schema.map
...
schemas.zaonhe = function (音韻地位, 字頭, 選項) {
